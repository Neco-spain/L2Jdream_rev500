 ...
 */
package com.dipedro.gameserver.taskmanager.tasks;
...
import com.dipedro.gameserver.taskmanager.Task;
import com.dipedro.gameserver.taskmanager.TaskManager;
import com.dipedro.gameserver.taskmanager.TaskManager.ExecutedTask;
import com.dipedro.gameserver.taskmanager.TaskTypes;
			...
			for(L2Clan c : ClanTable.getInstance().getClans())
			{
				for(Map.Entry<Integer, Integer> entry : rankList.entrySet())
				{
					...
					if(entry.getValue() <= 100 && c.isMember(playerName))
					{
						...
						c.addReputationScore(c.getReputationScore() + reputation, true);
					}
				}
			}
